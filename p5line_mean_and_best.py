...
for algo, algostr in enumerate(['', '_LogReg', '_LinearSVM']):
    plot_title = dataset
    if len(algostr) > 2:
        plot_title += algostr
    else:
    ...
    for rf_depth in (5,10,15,20):
        ...
        with open(filesPath+'/'+dataset+'/Results_'+variant+'/leq'+str(pattern_max_size)+'/'+'RF_'+str(rf_depth)+'_'+scoring_function+algostr+'.csv') as csv_file:
            ...
            for row in csv_reader:
                ...
                if line_count < 120:
                    depth_list.append(float(rowStr[1]))
                    line_count+=1
        csv_file.close()
        
        List.append(depth_list)
        
        table = np.reshape(np.array(depth_list), (-1,5))
        print(table)
        ...
        plt.errorbar(x_str, mean_table,yerr=var_table, capsize=3,label=rf_depth)
        #plt.plot(x_str,max_table, 'o-', label=rf_depth)
        #plt.scatter(x_str,max_table, marker='_', color='b', s=1000)
        #plt.title('maxDepth ' + str(rf_depth))
    
    table = np.empty((0,5))
    for depth_list in List:
        table = np.vstack((table,np.reshape(np.array(depth_list),(-1,5))))
    max_table = np.max(table,axis=0)
    best_index = np.argmax(max_table)
    plt.plot(x_str,max_table, 'o-', label='max')
